...
describe('Non-nullable object array criteria', () => {
  it('should be searchable by HasElementThatMatches', () => {
    repository.findOne({
    ...
    // @ts-expect-error
    repository.findOne({ friends: SearchBy.ArrayExists() });

    // @ts-expect-error
    repository.findOne({ friends: SearchBy.ArrayDoesNotExist() });
    ...
    // @ts-expect-error
    repository.findOne({ friends: SearchBy.DoesNotEqual([{ friendName: 'friend1' }]) });
